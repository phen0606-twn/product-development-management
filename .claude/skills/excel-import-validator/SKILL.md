...
|-----------|---------|---------|
| **新事業銷售統計週報** | `sheetNames.includes('新事業銷售總覽')` | **週開始日**（從檔名自動偵測，fallbackDate） |
...
格式：`YYYYMMDD-MMDD新事業銷售統計.xlsx`  
範例：`20260525-0531新事業銷售統計.xlsx`

- 週開始日：前8碼 `20260525` → `2026-05-25`
- 週結束日：後4碼 `0531`（月份同前，若結束日 < 開始日則月份+1）
- `sold_at` = **週開始日**（e.g., `2026-05-25`）
- 系統自動從檔名偵測，無需手動輸入日期
...
| Sheet 名稱 | 處理方式 |
|-----------|---------|
| 新事業商品 | **跳過**（商品 lookup 表）|
| 各倉庫存相關（含「各倉」或「附加」） | **跳過** |
| 分店代號 | **跳過** |
| 新事業銷售總覽 | **跳過**（彙總表，用於偵測格式）|
| 新事業商品庫存總覽 | **跳過** |
| 各商品 sheet（極冰絲滑UV外套、冰絲涼感衣…） | ✅ **解析**（每 sheet = 一個商品分類）|

### 資料 Sheet 欄位（header 在 row 0）

| col | 欄位名 | 說明 |
|-----|--------|------|
| 0 | 日期 | 日期範圍字串（不使用，從檔名取） |
| 1 | 商品型號 | SKU（唯一識別用，空白列跳過） |
| 2 | 分店代號 | 門市代碼（e.g., `A063`）|
| 3 | 數量 | 銷售件數（負數 = 退貨，保留）|
| 4 | 應售金額 | 不使用 |
| 5 | 實售金額 | 實際業績（NaN/空白 → 0，負數保留）|
| 11 | 品名規格 | 商品名稱 |
| 12 | 分店名稱 | 門市名稱 |
| 13 | 分店 | 完整門市資訊（`分店代號 分店名稱`，傳給 `classifyStore()`）|
...
|---------|---|
| `external_sku` | col[1] |
| `external_product_name` | col[11] |
| `sold_at` | **週開始日**（e.g., `2026-05-25`）|
| `quantity` | col[3] |
| `revenue` | col[5]（NaN/空 → 0）|
| `channel` | `classifyStore(col[13])` |
| `notes` | col[13]（分店完整名稱，供查閱）|
...
| 總銷售件數 | **742 件** |
| 總業績金額 | **$596,422** |
| 商品 sheet 數 | 16 個有效（3 個空 sheet 跳過）|

### 匯入結果範例

```
📅 週別：2026-05-25 ～ 2026-05-31　已解析 551 筆｜742 件｜$596,422
✅ 匯入完成｜🟢 新增 551 筆（2026-05-25）　📊 總計 742 件 / $596,422
```

### 冪等性機制

與其他格式相同：`DELETE WHERE sold_at = weekStart` → `INSERT`  
同週重新上傳時，刪除舊有 551 筆，重新插入新 551 筆，不會累加。
...
| 2026-06-10 | v2.1 | 新增格式 D：新事業銷售統計週報；parseWeeklySales；檔名自動偵測週別；週趨勢圖 X 軸改為週範圍格式（M/DD-M/DD）|
