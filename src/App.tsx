...
const nav = [
  ...
  ['/reorder-alert', '追貨警示', AlertTriangle, false],
    ...
    <div className="min-h-screen bg-mist lg:grid lg:grid-cols-[238px_1fr]">
      ...
      <main className="p-5 lg:p-8">
        <Routes>
          ...
          <Route path="/reorder-alert" element={<ErrorBoundary><ReorderAlertPage /></ErrorBoundary>} />
...
const MONTHLY_TARGETS_2026: Record<string, Record<number, number>> = {
  AS1SG: { 1:280000, 2:200000, 3:250000, 4:470000, 5:710000, 6:900000, 7:905000, 8:820000, 9:800000, 10:235000, 11:105000, 12:55000 },
  AS1SB: { 1:0, 2:0, 3:20000, 4:80000, 5:150000, 6:200000, 7:200000, 8:180000, 9:150000, 10:50000, 11:50000, 12:0 },
  AS1SE: { 1:0, 2:0, 3:0, 4:92000, 5:138000, 6:180000, 7:180000, 8:135000, 9:90000, 10:45000, 11:45000, 12:0 },
  AS1SC: { 1:0, 2:0, 3:0, 4:400000, 5:600000, 6:720000, 7:720000, 8:650000, 9:630000, 10:170000, 11:120000, 12:0 },
  AH1HC: { 1:2028000, 2:1134000, 3:213000, 4:0, 5:0, 6:0, 7:0, 8:0, 9:0, 10:45000, 11:325000, 12:3022000 },
  AH1HE: { 1:1570000, 2:1116000, 3:167000, 4:0, 5:0, 6:0, 7:0, 8:0, 9:0, 10:167000, 11:1116000, 12:672000 },
};
...
function ChannelSummary({ rows }: { rows: Array<{ label: string; quantity: number; revenue: number }> }) {
  ...
  const STORE_LABELS = new Set(['街邊店', '捷運門市', '加盟門市']);
  const storeTotal = rows.filter(r => STORE_LABELS.has(r.label)).reduce((s, r) => s + r.revenue, 0);
  let cursor = 0;
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
      <h3 className="mb-4 font-semibold">通路業績</h3>
      <div className="space-y-3">
        {rows.map((r, i) => (
          <div key={r.label} className="grid grid-cols-[1fr_auto] gap-3 rounded-md border p-3 text-sm">
            <p>
              <span className="mr-2 inline-block h-3 w-3 rounded-full" style={{ backgroundColor: getColor(r.label, i) }} />
              {r.label}
              <span className="ml-2 text-slate-500">{total ? (r.revenue / total * 100).toFixed(1) : 0}%</span>
            </p>
            <p className="font-semibold text-leaf">{formatCurrency(r.revenue)}</p>
          </div>
        ))}
        {storeTotal > 0 && (
          <div className="grid grid-cols-[1fr_auto] gap-3 rounded-md border border-slate-300 bg-slate-50 p-3 text-sm">
            <p className="font-medium text-slate-700">所有門市合計（街邊店＋捷運門市＋加盟門市）</p>
            <p className="font-semibold text-leaf">{formatCurrency(storeTotal)}</p>
          </div>
        )}
      </div>
      <div className="grid gap-5 border-t pt-5 md:grid-cols-[260px_1fr] md:items-center">
        <div className="mx-auto grid h-60 w-60 place-items-center rounded-full" style={{ background: `conic-gradient(${gradient})` }}>
          <div className="grid h-28 w-28 place-items-center rounded-full bg-white shadow-sm">
            <p className="text-center text-sm font-semibold">{formatCurrency(total)}</p>
          </div>
        </div>
        <div>
          {rows.map((r, i) => (
            <p key={r.label} className="mb-2 text-sm">
              <span className="mr-2 inline-block h-3 w-3 rounded-full" style={{ backgroundColor: getColor(r.label, i) }} />
              {r.label}
            </p>
          ))}
...
const HS_PRESETS = [
  ...
  { code: '9004.10.00.90-7', name: '太陽眼鏡',               rate: 5    },
...
function ReorderAlertPage() {
  ...
  const latestBySku = useMemo(() => {
    ...
    const totals = new Map<string, { external_sku: string; quantity: number }>();
    for (const r of inventory.rows) {
      const sku = String(r.external_sku || '');
      const date = String(r.recorded_at || '').slice(0, 10);
      if (!sku || date !== globalLatest) continue;
      const entry = totals.get(sku) ?? { external_sku: sku, quantity: 0 };
      entry.quantity += Number(r.quantity ?? 0);
      totals.set(sku, entry);
    }
    return [...totals.values()];
  ...
  return (
    <Page title="追貨警示" subtitle="太陽眼鏡庫存週轉分析、追貨建議與團購備貨試算">
      {/* ── 太陽眼鏡追貨警示 ─────────────────────────────────────────── */}
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
